...
class TicketButton(discord.ui.View):
    ...
    async def start_interview(self, channel, user, guild):
        ...
        try:
            ...
            # 🔥 [ส่วนเปลี่ยนชื่อ] ชื่อเดิม (ชื่อเล่น) 🔥
            try:
                original_name = user.display_name # ชื่อเดิม
                intro_name = data['name']         # ชื่อเล่นใหม่
                
                # จัดรูปแบบ
                new_nickname = f"{original_name} ({intro_name})"

                # ตัดให้ไม่เกิน 32 ตัวอักษร
                ...
                await channel.send("⚠️ **แจ้งเตือน:** บอทเปลี่ยนชื่อให้ไม่ได้ (ยศคุณสูงกว่าบอท หรือคุณเป็นเจ้าของดิส)")
            except Exception as e:
...
# ==========================================
# ⚡ ระบบ Slash Command (เมนูเด้ง) & แอดมิน
# ==========================================
...
async def fortune(interaction: discord.Interaction):
    ...
    fortunes = [
        ...
        "📈 **ดวงกลางๆ** พอถูไถ แต่อย่าหวังของแรร์เลย",
        "💎 **มีแววเสียตังค์ฟรี** เปอร์เซ็นต์สำเร็จ 99% = แตก (ตามสูตรเกม)",
        "✨ **แสงสีทองรออยู่!** (ในฝันนะ) ของจริงน่าจะได้แค่พลุ",
...
# 2. ล้างข้อความ (Clear)
@bot.tree.command(name="ล้าง", description="🧹 ลบข้อความล่าสุด (ระบุจำนวนได้)")
@app_commands.describe(amount="จำนวนข้อความที่ต้องการลบ (เช่น 10)")
@app_commands.checks.has_permissions(manage_messages=True)
async def clear_chat(interaction: discord.Interaction, amount: int):
    if amount > 100:
        await interaction.response.send_message("❌ ลบได้สูงสุดครั้งละ 100 ข้อความครับ", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    try:
        deleted = await interaction.channel.purge(limit=amount)
        await interaction.followup.send(f"🧹 ลบข้อความไปแล้ว {len(deleted)} ข้อความครับ!", ephemeral=True)
    except Exception as e:
        await interaction.followup.send(f"❌ ลบไม่ได้ (ข้อความอาจเก่าเกิน 14 วัน): {e}", ephemeral=True)

# 3. ล้างห้อง (Nuke)
@bot.tree.command(name="ล้างห้อง", description="⚠️ ลบห้องนี้ทิ้งแล้วสร้างใหม่ (แชทหายเกลี้ยง 100%)")
@app_commands.checks.has_permissions(administrator=True)
async def nuke_channel(interaction: discord.Interaction):
    embed = discord.Embed(
        title="⚠️ ยืนยันการล้างห้อง (Nuke)",
        description="คุณแน่ใจไหม? **ห้องนี้จะถูกลบและสร้างใหม่**\nประวัติการคุยทั้งหมดจะหายไปกู้คืนไม่ได้!",
        color=0xff0000
    )
    view = discord.ui.View()
    async def confirm_callback(bt_interaction):
        if bt_interaction.user != interaction.user: return
        await bt_interaction.response.send_message("💣 กำลังระเบิดห้องใน 3... 2... 1...", ephemeral=True)
        original_channel = interaction.channel
        new_channel = await original_channel.clone(reason="Nuke Channel by Bot")
        await original_channel.delete()
        await new_channel.send("https://media.giphy.com/media/XUFPGR4jHVXBC/giphy.gif")
        await new_channel.send(f"✨ **ห้องใหม่ไฉไลกว่าเดิม!** (ล้างบางโดย {interaction.user.mention})")

    btn_confirm = discord.ui.Button(label="ยืนยันระเบิดห้อง 💣", style=discord.ButtonStyle.danger)
...
async def clear_error(interaction: discord.Interaction, error):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("⛔ คุณไม่มีสิทธิ์ใช้คำสั่งนี้นะครับ (ต้องเป็น Admin/Mod)", ephemeral=True)
